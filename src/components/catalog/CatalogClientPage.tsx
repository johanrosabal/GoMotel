        ...
        <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="border rounded-lg p-4">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <Button size="sm" onClick={() => { setEditingCategory(undefined); setCategoryDialogOpen(true); }} id="catalogclientpage-button-a-adir" data-testid="catalogclientpage-add-button">
                                ...
                                <div key={cat.id} onClick={() => handleSelectCategory(cat)} className={cn("p-2 rounded-md cursor-pointer group", selectedCategory?.id === cat.id ? 'bg-primary text-primary-foreground' : 'hover:bg-accent')}>
                                    <div className="flex justify-between items-center">
                                        ...
                                        <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                                            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={(e) => {e.stopPropagation(); setEditingCategory(cat); setCategoryDialogOpen(true);}} id="catalogclientpage-button-1" data-testid="catalogclientpage-edit-button"><Edit className="h-4 w-4"/></Button>
                                            <DeleteAction id={cat.id} name={cat.name} type="categoría" onDelete={handleDeleteCategory}>
                                                <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive" onClick={(e) => e.stopPropagation()} id="catalogclientpage-button-2" data-testid="catalogclientpage-delete-button"><Trash2 className="h-4 w-4"/></Button>
                                            </DeleteAction>
                                        </div>
                                    </div>
                                </div>
            ...
            <div className="border rounded-lg p-4">
                 <div className="flex justify-between items-center mb-4">
                    ...
                    <Button size="sm" disabled={!selectedCategory} onClick={() => { setEditingSubCategory(undefined); setSubCategoryDialogOpen(true); }} id="catalogclientpage-button-a-adir-1" data-testid="catalogclientpage-add-button">
                        <PlusCircle className="mr-2 h-4 w-4" /> Añadir
                    </Button>
                </div>
                <ScrollArea className="h-96">
                             ...
                             {subCategories?.map(sub => (
                                <div key={sub.id} onClick={() => setSelectedSubCategory(sub)} className={cn("p-2 rounded-md cursor-pointer group", selectedSubCategory?.id === sub.id ? 'bg-accent text-accent-foreground' : 'hover:bg-accent')}>
                                   <div className="flex justify-between items-center">
                                        ...
                                        <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                                            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={(e) => { e.stopPropagation(); setEditingSubCategory(sub); setSubCategoryDialogOpen(true); }} id="catalogclientpage-button-3" data-testid="catalogclientpage-edit-button">
                                                <Edit className="h-4 w-4"/>
                                            </Button>
                                            <DeleteAction id={sub.id} name={sub.name} type="sub-categoría" onDelete={handleDeleteSubCategory}>
                                                <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive" onClick={(e) => e.stopPropagation()} id="catalogclientpage-button-4" data-testid="catalogclientpage-delete-button">
            ...
            <div className="border rounded-lg p-4">
                 <div className="flex justify-between items-center mb-4">
                    ...
                    <Button size="sm" disabled={!selectedSubCategory} onClick={() => { setEditingService(undefined); setServiceDialogOpen(true); }} id="catalogclientpage-button-a-adir-2" data-testid="catalogclientpage-add-button">
                        <PlusCircle className="mr-2 h-4 w-4" /> Añadir
                    </Button>
                </div>
                 <ScrollArea className="h-96">
                             ...
                             {services?.map(srv => (
                                <div key={srv.id} className="p-2 rounded-md hover:bg-muted group">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                                            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={(e) => { e.stopPropagation(); setEditingService(srv); setServiceDialogOpen(true); }} id="catalogclientpage-button-5" data-testid="catalogclientpage-edit-button">
                                                <Edit className="h-4 w-4"/>
                                            </Button>
                                            <DeleteAction id={srv.id} name={srv.name} type="producto" onDelete={handleDeleteService}>
                                                <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive" onClick={(e) => e.stopPropagation()} id="catalogclientpage-button-6" data-testid="catalogclientpage-delete-button">
                                                    <Trash2 className="h-4 w-4"/>
                                                </Button>
                                            </DeleteAction>
                                        </div>
