    ...
    <header className="sticky top-0 z-30 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container flex h-16 items-center">
            <div className="flex flex-1 items-center justify-start gap-2 md:gap-4">
              {showBackButton ? (
                  <Button variant="secondary" className="h-9 rounded-md font-bold px-3 border-primary/10 shadow-sm transition-all hover:bg-secondary/80" asChild id="topnav-button-1" data-testid="topnav-action-button">
                    <Link href="/dashboard" id="topnav-link-dashboard" data-testid="topnav-back-link">
            ...
            <div className="hidden sm:flex flex-none items-center justify-center mx-4">
              <Link href={user ? '/dashboard' : '/'} className="flex items-center gap-2 group" id="topnav-link-1" data-testid="topnav-action-link">
                  {company?.logoUrl ? (
