      ...
      {navSections.filter(section => section.roles.includes(userRole)).map((section) => (
          <div key={section.title} className="space-y-4">
              ...
              {section.links.filter(link => !link.roles || link.roles.includes(userRole)).map((link) => (
                  <Link key={link.title} href={link.href} className="group relative" target={link.href.startsWith('/public') ? "_blank" : "_self"} id={`dashboard-link-${link.href.replaceAll('/', '-').replace(/^-/, '')}`} data-testid="dashboard-link-1">
                      {link.badge === 'PASO 1' && (
